...
import com.android.server.frankenstein.FrankensteinFrameworkHooks;
...
final class InputManagerCallback implements InputManagerService.WindowManagerCallbacks {
    ...
    public void notifyFocusChanged(IBinder oldToken, IBinder newToken) {
        ...
        String packageName = "";
        synchronized (mService.mGlobalLock) {
            WindowState target = mService.mInputToWindowMap.get(newToken);
            if (target != null) {
                uid = target.mOwnerUid;
                pid = target.mSession.mPid;
                userId = UserHandle.getUserId(uid);
                displayId = target.getDisplayId();
                packageName = target.getOwningPackage();
            }
        }
        FrankensteinFrameworkHooks.focusChanged(
                userId, uid, pid, displayId, packageName, newToken != null);
